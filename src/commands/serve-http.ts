...
import { chmodSync as koaChmod, readFileSync, writeFileSync as koaWriteFile, mkdirSync as koaMkdir } from 'node:fs';
import { join, dirname as koaDirname } from 'node:path';
...
export async function runServeHttp(engine: BrainEngine, options: ServeHttpOptions) {
  ...
  const koaAdminSessFile = process.env.GBRAIN_ADMIN_SESSIONS_FILE
    || join(koaHomedir(), '.gbrain', 'admin-sessions.json');
  const koaAdminSessDir = koaDirname(koaAdminSessFile);
  const adminSessions: Map<string, number> = (() => {
    const sessions = new Map<string, number>();
    try {
      koaMkdir(koaAdminSessDir, { recursive: true, mode: 0o700 });
      koaChmod(koaAdminSessDir, 0o700);
      koaChmod(koaAdminSessFile, 0o600);
      const obj = JSON.parse(readFileSync(koaAdminSessFile, 'utf8')) as Record<string, number>;
      const now = Date.now();
      for (const [key, expiresAt] of Object.entries(obj)) {
        if (typeof expiresAt === 'number' && expiresAt > now) sessions.set(key, expiresAt);
      }
    } catch { /* no usable session file yet */ }
    const persist = () => {
      try {
        koaMkdir(koaAdminSessDir, { recursive: true, mode: 0o700 });
        koaChmod(koaAdminSessDir, 0o700);
        const obj: Record<string, number> = {};
        for (const [key, expiresAt] of sessions) obj[key] = expiresAt;
        koaWriteFile(koaAdminSessFile, JSON.stringify(obj), { mode: 0o600 });
        koaChmod(koaAdminSessFile, 0o600);
      } catch { /* auth stays available if persistence fails */ }
    };
    const set = sessions.set.bind(sessions);
    const del = sessions.delete.bind(sessions);
    const clear = sessions.clear.bind(sessions);
    sessions.set = (key: string, expiresAt: number) => { const result = set(key, expiresAt); persist(); return result; };
    sessions.delete = (key: string) => { const result = del(key); persist(); return result; };
    sessions.clear = () => { clear(); persist(); };
    return sessions;
  })(); // sessionId -> expiresAt

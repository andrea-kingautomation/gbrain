  ...
  for (const seg of segments) {
    ...
    for (let ci = 0; ci < chunks.length; ci++) {
      ...
      if (!extraction.ok) {
        ...
        if (extraction.reason === 'provider_error') {
          const cooldown = detail.match(/retry after (\d+)s/i);
          const cooldownKey = `${seg.startIso}|${ci}`;
          const tried = cooldownRetries.get(cooldownKey) ?? 0;
          if (cooldown && tried < 2 && !state.signal?.aborted) {
            cooldownRetries.set(cooldownKey, tried + 1);
            const waitSeconds = Math.min(Number(cooldown[1]) + 3, 180);
            process.stderr.write(
              `[extract-conversation-facts] router cooldown - waiting ${waitSeconds}s then retrying chunk ${ci + 1}/${chunks.length} (attempt ${tried + 1}/2)\n`,
            );
          ...
          if (state.consecutiveCallErrors >= LANE_OUTAGE_BREAKER_LIMIT) {
            throw new LaneOutageBreaker();
          }
        }
        throw new Error(
          `segment ${seg.startIso}..${seg.endIso} chunk ${ci + 1}/${chunks.length} extraction failed (${extraction.reason})${detail ? `: ${detail}` : ''}`,
        );
      }
      state.consecutiveCallErrors = 0;

...
export async function buildChecks(
  ...
  //
  // Surfaces the fraction of linkable ENTITY-GRAPH pages with no inbound links.
  // The broad `gbrain orphans` inventory intentionally includes notes,
  // conversations, atoms, receipts, and other page classes. Doctor health must
  // match the entity-count gate below, so it scopes the shared data helper to
  // LINKABLE_ENTITY_TYPES without changing broad orphan inventory semantics.
  //
  ...
  try {
    ...
    const { LINKABLE_ENTITY_TYPES } = await import('../core/by-mention.ts');
    ...
    if (entityCount < 100 && !srcId) {
      ...
      const data = await getOrphansData(engine, {
        includePseudo: false,
        sourceId: srcId,
        pageTypes: LINKABLE_ENTITY_TYPES,
      });
      ...
      if (ratio > 0.8) {
        checks.push({
          name: 'orphan_ratio',
          status: 'fail',
          message: `Entity orphan ratio ${pct}%${inSource} (${data.total_orphans}/${data.total_linkable} entity pages have no inbound links)${caveat}. ${hint}`,
        });
      } else if (ratio > 0.5) {
        checks.push({
          name: 'orphan_ratio',
        ...
        checks.push({
          name: 'orphan_ratio',
          status: 'ok',
          message: `Entity orphan ratio ${pct}%${inSource} (${data.total_orphans}/${data.total_linkable} entity pages)${caveat}`,
        });
      }
    }
  } catch {

...
_gbrain_git() {
  _file_allow=never
  [ "\${GBRAIN_GIT_ALLOW_FILE_TRANSPORT:-0}" = "1" ] && _file_allow=always
  git -c http.followRedirects=false -c "protocol.file.allow=$_file_allow" -c protocol.ext.allow=never "$@"
}
brain_push() {
  _branch="$1"
  ...
  _gd="$(_gbrain_git rev-parse --git-dir 2>/dev/null || echo .git)"
  ...
  if _gbrain_git push origin "HEAD:$_branch" >>"$_log" 2>&1; then
    echo "$(date -u +%FT%TZ) [push] ok $_branch $(_gbrain_git rev-parse --short HEAD 2>/dev/null)" >>"$_log"; return 0
  fi
  echo "$(date -u +%FT%TZ) [push] rejected; rebase-pull $_branch" >>"$_log"
  if _gbrain_git pull --rebase origin "$_branch" >>"$_log" 2>&1 && _gbrain_git push origin "HEAD:$_branch" >>"$_log" 2>&1; then
    echo "$(date -u +%FT%TZ) [push] ok-after-rebase $_branch $(_gbrain_git rev-parse --short HEAD 2>/dev/null)" >>"$_log"; return 0
  fi
  _gbrain_git rebase --abort >/dev/null 2>&1 || true
  echo "$(date -u +%FT%TZ) [push] LOCAL-ONLY, NEEDS ATTENTION: $_branch @ $(_gbrain_git rev-parse --short HEAD 2>/dev/null) could not reach origin. Run: gbrain sources pull <id> && git push" >>"$_log"
  return 1
}`;
...
function commitScaffolding(repoPath: string, branch: string, redact: (s: string) => string): { status: StepStatus; detail: string } | null {
  ...
  try {
    ...
    execFileSync(join(repoPath, HELPER_REL), ['--push-only', branch], {
      cwd: repoPath,

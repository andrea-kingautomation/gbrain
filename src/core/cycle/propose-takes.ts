...
- Questions, greetings, or small talk ("Are you up?", "And you?", "Think hard")
- Test messages or filler ("this is a test", "testing 123")
- Requests, instructions, or commands to an assistant ("I want you to fetch...", "please summarise...", "let's...")
- Statements about the SYSTEM'S OWN operation, runtime, logs, tooling, token usage,
  restarts, ports, model availability, or process metrics ("We extracted 1727 facts...",
  "tokens loaded is in respect to context window", "MCP will load after restart",
  "deepseek returning 429"). These describe the machine, not the world.
...
/**
 * Page types that are operational artifacts, never a source of gradeable
 * opinions. `extract_receipt` pages are gbrain's own process logs (fact/take
 * extraction receipts) and were leaking junk proposals like
 * "We extracted 1727 facts from 42 out of 98 eligible pages." (proposer
 * quality fix 2026-06-07).
 */
export const NON_KNOWLEDGE_PAGE_TYPES = new Set<string>(['extract_receipt']);

/** Explicit test/greeting/small-talk strings that are never gradeable claims. */
const JUNK_CLAIM_RX = /^(?:this is a test|testing\b|test message|are you (?:up|there|awake)|and you(?:\?|$)|you up|think hard|good morning|good night|thanks?|thank you|ok(?:ay)?|hello|hi|hey)\b/i;

/** Imperative/request openers — a command to an assistant, not an opinion. */
const REQUEST_OPENER_RX = /^(?:i (?:want|need|would like) you to|can you|could you|please|let'?s|go ahead|make sure you|remember to)\b/i;

/** System self-reference — describes the machine's own runtime, not the world. */
const SYSTEM_SELF_RX = /\b(?:after restart|will load|loads? (?:after|successfully)|reliably after|returning \d{3}\b|\bport \d{3,5}\b|tokens? loaded|context window|mcp (?:is|will|not)|restart with updated|notebooklm tools|429 from)\b/i;

/**
 * Deterministic low-quality gate applied to every parsed proposal before it is
 * written to take_proposals. Rejects questions, greetings, test strings,
 * requests/commands, system-runtime chatter, and fragments too short to carry a
 * gradeable claim. Conservative by design — real opinions are declarative and
 * specific, so these patterns do not catch them. (proposer quality fix 2026-06-07)
 */
export function isLowQualityClaim(text: string): boolean {
  const t = (text ?? '').trim();
  if (!t) return true;
  if (t.endsWith('?')) return true;                       // questions aren't claims
  const words = t.split(/\s+/).filter(Boolean);
  if (words.length < 4 || t.length < 20) return true;     // greetings / fragments
  if (JUNK_CLAIM_RX.test(t)) return true;
  if (REQUEST_OPENER_RX.test(t)) return true;
  if (SYSTEM_SELF_RX.test(t)) return true;
  return false;
}
  ...
  protected async process(
    ...
    for (const page of pages) {
      ...
      for (const p of proposals) {
        if (isLowQualityClaim(p.claim_text)) {
          result.warnings.push(`dropped low-quality proposal on ${page.slug}: ${JSON.stringify(p.claim_text.slice(0, 60))}`);
          continue;
        }
    ...
    // Derive the receipt's source from the scope, else from the pages just
    // processed (a scoped run shares one source). Crucially, do NOT fall back
    // to the federated `default` source: a source-less run that wrote its
    // receipt/rollup to `default` would leak operational metadata across the
    // personal/business wall (this is exactly how a takes.proposed/personal
    // receipt once became globally readable). If no real source resolves, skip
    // these operational writes and surface it — the proposals themselves are
    // already correctly source-tagged from page.source_id above.
    const sourceIdForReceipt =
      scope.sourceId ?? pages.find((p) => p.source_id)?.source_id ?? '';
    if (!sourceIdForReceipt) {
      if (result.proposals_inserted > 0)
        console.error(
          `[propose_takes] no source scope resolved for ${result.proposals_inserted} proposals — skipping receipt + rollup to avoid writing into the federated default source`,
        );
    } else {
      if (result.proposals_inserted > 0) {
        try {

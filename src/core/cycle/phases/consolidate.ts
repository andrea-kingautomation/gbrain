...
export async function runPhaseConsolidate(
  ...
  for (const b of buckets) {
    ...
    // Resolve entity_slug → page_id. Entity pages live in the `default`
    // source while facts are extracted into per-source corpora
    // (fathom-meetings, koa-conversations, ...). Resolve CROSS-SOURCE:
    // prefer the fact's own source, then `default`, then any source — else
    // every meeting/conversation fact bucket fails to resolve and 0 takes
    // are ever promoted. (cross-source fix 2026-06-07, same class as the
    // timeline-from-meetings bug.)
    const pageRows = await engine.executeRaw<{ id: number }>(
      `SELECT id FROM pages WHERE slug = $2 AND deleted_at IS NULL
       ORDER BY (source_id = $1) DESC, (source_id = 'default') DESC, id ASC
       LIMIT 1`,

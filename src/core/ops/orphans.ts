...
const find_orphans: Operation = {
  ...
  params: {
    ...
    page_types: {
      type: 'array',
      items: { type: 'string' },
      description: 'Optional candidate page-type filter. Omit for the broad orphan inventory.',
    },
  },
  scope: 'read',
  handler: async (ctx, p) => {
    ...
    const result = await findOrphans(ctx.engine, {
      ...
      ...(Array.isArray(p.page_types)
        ? { pageTypes: p.page_types.filter((value): value is string => typeof value === 'string') }
        : {}),
      ...scope,
    });

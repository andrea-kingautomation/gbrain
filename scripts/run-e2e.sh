...
for f in "${files[@]}"; do
  ...
  FILE_HOME="$E2E_TMP_HOME/file-${file_idx:-$name}"
  mkdir -p "$FILE_HOME/.gbrain"
  if output=$(HOME="$FILE_HOME" GBRAIN_HOME="$FILE_HOME" $TIMEOUT_CMD bun test --timeout=60000 ${COVERAGE_ARGS[@]+"${COVERAGE_ARGS[@]}"} "$f" 2>&1); then

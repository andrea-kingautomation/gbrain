...
describe('runPhaseProposeTakes — phase integration', () => {
  ...
  test('#2138: multi-claim page inserts every claim with a per-claim conflict target', async () => {
    ...
    const { engine, captured } = buildMockEngine({ pages });
    const extractor: ProposeTakesExtractor = async () => [
      { claim_text: 'Network effects compound marketplace liquidity over time', kind: 'take', holder: 'brain', weight: 0.6 },
      { claim_text: 'Focused distribution creates stronger early product retention', kind: 'bet', holder: 'brain', weight: 0.8 },
    ];
    const result = await runPhaseProposeTakes(buildCtx(engine), { extractor });
    ...
    expect(inserts.map(i => i.params[5])).toEqual([
      'Network effects compound marketplace liquidity over time',
      'Focused distribution creates stronger early product retention',
    ]);
  });

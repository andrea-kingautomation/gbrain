...
describe('hardenBrainRepo', () => {
  ...
  test('scaffolding push reuses the serialized durability helper (#3925)', () => {
    const source = readFileSync(join(import.meta.dir, '..', 'src', 'core', 'brain-repo-durability.ts'), 'utf-8');
    const start = source.indexOf('function commitScaffolding(');
    const end = source.indexOf('function headMatchesOrigin(', start);
    expect(start).toBeGreaterThanOrEqual(0);
    expect(end).toBeGreaterThan(start);
    const body = source.slice(start, end);
    expect(body).toContain("execFileSync(join(repoPath, HELPER_REL), ['--push-only', branch]");
    expect(body).toContain('cwd: repoPath');
    expect(body).not.toContain("'push', 'origin'");
    expect(source).toContain('_gbrain_git push origin "HEAD:$_branch"');
    expect(source).toContain('protocol.file.allow=$_file_allow');
    expect(source).toContain('GBRAIN_GIT_ALLOW_FILE_TRANSPORT');
  });
